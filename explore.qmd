...
[calcofi.io/species/](https://calcofi.io/species/) and
[calcofi.io/measurements/](https://calcofi.io/measurements/) are the index — every organism or
measurement, which datasets carry it, over what years; **the Explorer is the map**, opening
already filtered on `?taxon=` or `?var=`.
...
floor (@fig-explore-section-3d), with the station grid at the surface for reference: every station
of the standard pattern as a dot with its number, a thread through each line and the line's name at
both ends. The camera is driven by dragging (shift-drag or right-drag rotates and tilts), by the
rotate, tilt and compass buttons that join the map's top-right row in 3-D (the compass resets the
line's own framing), or by the keyboard once the scene has been clicked (arrows pan, shift + arrows
rotate and tilt, + and − zoom). The vantage travels in the URL as `cam=` beside the vertical
exaggeration, so a shared link reopens at the same angle; **2D** at the top right brings the flat
one back.

![The same depth section as a 3-D curtain over the GEBCO sea floor, the standard station pattern threaded and labelled at the surface. Desktop and ocean variables only; the 2D button restores the flat section.](figs/explore/sections_3d.png){#fig-explore-section-3d .column-body-outset}
...
- **Basemap labels** — the basemap's own place names, road names, points of interest and water
  names, toned down so they never outrank the data (one grey, Regular weight, an opacity slider,
  `basemapo=`), plus the island names the basemap carries but never drew; off (`basemap=nolabels`)
  for a data-centric view, with the isobath labels and the undersea feature names untouched. The names layer has its own opacity in its *On the map* row.
- **On the map** — the boundary layers *and* the Data row in draw order, top first, re-ordered by
  ...
  above it draws the data over the boundary. The Data row is the sea surface: it and every row
  below it are clipped to the ocean by the coast, rows above it draw over the land too.
- **Add a layer** — the boundary registry by group: maritime zones, protected areas,
  ...
  reference. The names are on from the start, above the data and absent from the legend, with
  their opacity slider under the row; remove the row to turn them off (`layers=off` in the link).

The coast itself is not a layer of the card: it is always drawn over the sea floor and the data
from OpenStreetMap's land polygons (the same coastline the basemap's water comes from), so a 390 m
bathymetry cell or a 10 km contoured cell never spills onto land, and the basemap's roads, county,
state and country lines and labels stay on top.

Every choice — which layers, their order, their colour, fill opacity and line width, the sea
floor's parts and opacity, the data ramp — is view state and lives in the URL, so a
...
tibble::tribble(
  ...
  "the section",            "line, anom, cruise, view, exag, cam",    "line=90&anom=1&view=3d&cam=-121.2,32.6,7.6,55,-40",
