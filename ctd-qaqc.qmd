...
For the CTD team. The casts you quality-control can live in two places on purpose: a **working
...
**Available, not yet adopted.** The store is built, loaded and working — but it is not yet the
team's QA/QC practice, so today CalCOFI's CTD flags reach the release the way every other
dataset's do: inside the provider's own files. That is why the system figure in
[Architecture](architecture.qmd) does not show it, and why this chapter has its own
(@fig-system-ctd). Nothing below is a plan; it all runs today.
:::

## Where the store sits in the system

![The whole system with the CTD working store in it: the team proposes and reviews flags in `ctd.flag`, the server writes the accepted rows to `flag_accepted.parquet` every night, and the next CTD ingest applies them as `measurement_qual`. The pipeline never reads the live database — only that file — and `pg_duckdb` lets the store read the release back. Compare the overview figure in [Architecture](architecture.qmd), which is the same system without this box.](diagrams/system_ctd.svg){#fig-system-ctd .cc-diagram .nolightbox}

@fig-system-ctd is the ingest → integrate → publish path of the overview figure with one box
added on the left. **What is built and running**, since 2026-08-19: PostgreSQL 18 with PostGIS
3.6 and `pg_duckdb` on the CalCOFI server; 409 cast files and **10,812,360 scans** loaded
verbatim in all 82 source columns; the `ctd.flag` ledger with its audit trail and the
`v_scan_qc` / `v_scan_clean` views over it; 20,521 best-stage casts, 1993–2026; personal
accounts, a tunnel and clients in R and Python ([Server access](server-access.qmd)); and the
nightly job that writes every accepted flag to
`gs://calcofi-db/qc/ctd/flag_accepted.parquet`.

**The pipeline never reads the live database.** The bridge is that one parquet file, so an
ingest runs on a laptop, on the server or in CI without a tunnel, a credential or a database
being up — and the flags it applies are a dated, immutable snapshot rather than whatever the
ledger happened to say mid-run.

**What adopting it would change**: a flag accepted in the ledger reaches the next release on its
own, with nobody touching the release, re-exporting a cast file or asking the data team to
re-ingest anything — and the reasoning behind each flag stays in the database, attributable and
reversible. **What happens instead today**: flags arrive with the provider's files, so a
correction waits for the next archive delivery, and the judgement behind it lives wherever the
person who made it kept it. Both paths end in the same column, `measurement_qual`, uninterpreted.
...
@fig-ctd-flow is the store itself, close up, and @tbl-ctd-schemas is what it holds. Database
`calcofi`, three kinds of schema:
...
: The schemas of database `calcofi` and who writes to each; [Server access](server-access.qmd) has the longer form. {#tbl-ctd-schemas}
...
4. **It reaches the release.** This is the step that waits on adoption, and it needs nothing
   built. Every night the server writes the accepted rows to
